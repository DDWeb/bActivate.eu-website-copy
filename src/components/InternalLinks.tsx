...
interface LinkItem {
    ...
    label: string;
    description: string;
    plain?: boolean; // static page outside the router: render a plain <a>
}
        ...
        <section style={{ background: '#f8f8f8', borderTop: '1px solid #e5e7eb', padding: '3rem 1rem' }}>
            <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
                    {links.map((link, i) => {
                        const style = { textDecoration: 'none', display: 'block', background: 'white', padding: '1.25rem 1.5rem', borderLeft: '3px solid #901820', color: 'inherit' } as const;
                        const inner = (
                            <>
                        ...
                        return link.plain
                            ? <a key={i} href={link.to} style={style}>{inner}</a>
                            : <Link key={i} to={link.to} style={style}>{inner}</Link>;
                    })}
                </div>
            </div>
        </section>
